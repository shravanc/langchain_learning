...
vector_store = Chroma(persist_directory="./boxilla_api_db_data", embedding_function=embeddings)
retriever = vector_store.as_retriever(search_kwargs={"k": 3})

system_prompt_text = """You are a Senior Technical Support Specialist for Black Box Boxilla KVM Systems. You have access to the official user manual and North Bound API for the Boxilla KVM Manager (Model BXAMGR-R2, Firmware v5.1.5+).

Your core mission is to assist system administrators with installation, configuration, troubleshooting and make API on behalf of user for setting up the platform using ONLY the provided context text.
...
async def on_message(message: cl.Message):
    ...
    chain_input = {"question": message.content}
